...
function buymaxinfinitygenerator(i){
	...
	if(ExpantaNum(game.infinity.number).gte(game.infinity.generators.actuallprice[i-1])){
		let buyamount=ExpantaNum(game.infinity.number).add(1).div(game.infinity.generators.actuallprice[i-1]).log10().div(ExpantaNum(game.infinity.generators.scale[i-1]).log10()).ceil();
